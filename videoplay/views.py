...
def play(request, id=0):
    ...
    # 缓存视频
    utils.download(video_source.movie)
    # 渲染视图

    return HttpResponse(t.render({"user": sign, "video_source": video_source, "video_id": video_id,"movie_payment_obj": len(movie_payment_obj), "pay": pay, "url": url, "comments_list": comments_list,'movie':video_source.movie}))
...
#     comments_list = UserComment.objects.all()
#     # 获取数据库中已支付的对象
#     movie_payment_obj = MoviePay.objects.filter(movie_id = video_id)
#     # 支付 False：未购买 True: 已购买
#     pay = False
#     if video_id <= 5:
#         if len(movie_payment_obj) != 0:
...
#     # 加载视频播放模板
#     t = loader.get_template("video.html")
#     c = t.render({"user": 'TestUser', "video_source": video_source, "video_id": video_id,"movie_payment_obj": len(movie_payment_obj), "pay": pay, "url": url, "comments_list": comments_list})
...
def regist(request):
    if request.method == "POST":
        ...
        # print(uf_regist.cleaned_data.get('username'))
        # 校验提交数据是否合法
